...
Better models on the DR segmentation problem will be released. -> OK (shuntedSeg.py (I designed, not from scratch)) <br>
The final code will be released. -> OK <br>

## Important
The results in the .out files are based on the coefficients in the .py files. The coefficients (lambda's) should be adjusted for these models that use the shuntedSeg model. They are arbitrary (Actually, they are remaining from mambaVisionSeg (I designed, not from scratch)). <br>
