...
from utils.dataset_da_denseNet_Last_down import FundusSeg_Loader, FundusSeg_Loader_Test


dataset_name = "idrid" #

if dataset_name == "idrid":
    train_data_path = "data/idrid/train768x1024/"
    valid_data_path = "data/idrid/test768x1024/"
    N_epochs = 600
    lr_decay_step = [180, 360]
    lr_init = 0.00005
    batch_size = 8
    test_iter = 80
...
class Discriminator(nn.Module):
    ...
    def forward(self, map, feature):
        ...
        x = x.reshape(x.shape[0], self.in_features, 8, 8)
        x = self.classifier(x)
        x = self.avgpool(x)
...
def train_net(net, discriminator, discriminator2, device, epochs = N_epochs, batch_size = batch_size, lr = lr_init):
    ...
    eyePacs_path = "data/idrid/idrid_grade_768x1024/"
    ...
    batch_sampler = TwoStreamBatchSampler(list(range(516)), list(range(516, 516 + 54)), unLbl_bs, lbl_bs)
    ...
    optimizer_dsc = optim.Adam(discriminator.parameters(), lr = lr / 5, weight_decay = 1e-5)
    optimizer_dsc2 = optim.Adam(discriminator2.parameters(), lr = lr / 10, weight_decay = 1e-5)
    ...
    criterion = nn.CrossEntropyLoss(weight = torch.FloatTensor([1., 2, 1.5, 3, 0.8]).to(device))
    ...
    beforeSoftmax = True
    save_aucpr_metric_epoch = 80

    decreasing = False
    saver = ckptSaver.CheckpointSaver(
            model = net, optimizer = optimizer, args = None, model_ema = None, amp_scaler = loss_scaler,
            checkpoint_dir = 'ckpt_idrid_shuntedSeg256_add3_3mf_k4_600/', 
            recovery_dir = 'ckpt_idrid_shuntedSeg256_add3_3mf_k4_600/', decreasing = decreasing, max_history = 21)
    ...
    for epoch in range(epochs):
        ...
        for i, (image, label, image_down, label_down) in enumerate(train_loader):
            ...
            with amp_autocast():
                ...
                pred_unl128_soft_intrplt = F.interpolate(pred_unl128_soft, size = (256, 256), mode = 'bilinear', align_corners = True)
                image_down_intrplt = F.interpolate(image_down[:unLbl_bs - lbl_bs], size = (256, 256), mode = 'nearest')
                ...
                if beforeSoftmax == True:
                    ...
                    ## pred128_soft_intrplt = F.interpolate(pred128_soft, size = (256, 256), mode = 'bilinear', align_corners = True)
                    pred_unl128_soft_intrplt = F.interpolate(pred_unl128_soft, size = (256, 256), mode = 'bilinear', align_corners = True)
                    ## image_down_intrplt = F.interpolate(image_down, size = (256, 256), mode = 'nearest')
                    ...
                    pred128_intrplt = F.interpolate(pred128[0], size = (256, 256), mode = 'bilinear', align_corners = True)
                    pred_unl128_intrplt = F.interpolate(pred_unl128[0], size = (256, 256), mode = 'bilinear', align_corners = True)
                    image_down_intrplt = F.interpolate(image_down, size = (256, 256), mode = 'bilinear', align_corners = True)
                ...
                loss_2dec_lbl = F.mse_loss(out1_soft_lbl, F.interpolate(out2_soft_lbl, size = (256, 256), mode = 'bilinear', align_corners = True), reduction = 'mean')

                out1_soft_unlbl = torch.softmax(pred_unl[0], dim = 1)
                out2_soft_unlbl = torch.softmax(pred_unl128[0], dim = 1)
                loss_2dec_unlbl = F.mse_loss(out1_soft_unlbl, F.interpolate(out2_soft_unlbl, size = (256, 256), mode = 'bilinear', align_corners = True), reduction = 'mean')

                consistency_weight = get_current_consistency_weight(epoch)
                ## loss = loss_sp_256 + 0 * loss_sp_128 + 0 * loss_2dec_lbl + 0 * loss_2dec_unlbl + 0.008 * lossAdv + 0.05 * lossAdv2
                if epoch < 200:
