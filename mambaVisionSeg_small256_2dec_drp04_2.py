...
class SwinUMambaD(nn.Module):
    def __init__(self, vss_args, decoder_args):
        super().__init__()
        self.vssm_encoder = MambaVision(depths = [3, 3, 7, 5],
                        num_heads = [2, 4, 8, 16],
        ...
        state_dict = torch.load("/arf/home/osivaz/preweights/mambavision_small_1k.pth.tar", weights_only = False)
        self.vssm_encoder.load_state_dict(state_dict['state_dict'])
        ...
        self.decoder.apply(init_last_bn_before_add_to_0)

        self.decoder2 = UNetResDecoder(**decoder_args)
        self.decoder2.apply(InitWeights_He(1e-2))
        self.decoder2.apply(init_last_bn_before_add_to_0)

    def forward(self, x1, x2):
        skips = self.vssm_encoder(x1)
        if x2 is not None:
            skips_128 = self.vssm_encoder(x2)
...
def load_pretrained_ckpt(
    ...
    ckpt_path = "vssmtiny_dp01_ckpt_epoch_292.pth"
):
...
def get_swin_umamba_from_plans(
    ...
    if patch_size[0] == 384 and patch_size[1] == 640:
        ...
        depth_squeeze_factors = [1, 1, 1] if no_squeeze else [squeeze_factor, 1, 1]
    elif patch_size[0] == 640 and patch_size[1] == 640:
        depth_stride_configs = [2] * (n_stages_encoder - 2) + [4]
        depth_stride_output_sizes = [320, 1280, 1280]
        depth_squeeze_factors = [1, 1, 1] if no_squeeze else [squeeze_factor, 1, 1]
...
## model = get_swin_umamba_from_plans(None, 3, True, False).cuda()
## output = model(torch.randn(4, 3, 512, 512).cuda())
## print(len(output))
