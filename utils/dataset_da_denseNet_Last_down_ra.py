...
class FundusSeg_Loader(Dataset):
    def __init__(self, data_path, is_train, eyePacsTrainFiles, eyePacs_path):
        ...
        print('osivaz')
        self.crop_size = 256
        self.aa_params = {'translate_const': 230, 'img_mean': (124, 116, 104), 'interpolation': 3}
        self.randAugment = rand_augment_transform('rand-m9-mstd0.5-inc1', self.aa_params)
    ...
    def __getitem__(self, index):
        ...
        if self.is_train == 1:
            ...
            image, label = self.randAugment([Image.fromarray(np.array(image)), Image.fromarray(np.array(label))])
        ...
        image_down = T.ToTensor()(image_down)
        label_down = T.ToTensor()(label_down)

        return image, label.reshape(self.crop_size, self.crop_size), image_down, label_down.reshape(self.crop_size//2, self.crop_size//2)
