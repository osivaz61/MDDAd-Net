...
class FundusSeg_Loader(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.is_train == 1:
            ...
            if np.random.random_sample() <= 0.5:
                if ssl == 1:
                    image_L, image_H, label = self.randomRotation(image_L, image_H, label)
                else:
                    image_L, image_H, _ = self.randomRotation(image_L, image_H, image_L)
            ...
            if np.random.random_sample() <= 1:
                ...
                w = random.uniform(0, image_L.size[0] - self.crop_size)
                h = random.uniform(0, image_H.size[1] - self.crop_size)
                if ssl == 1:
                    image_L = TF.crop(image_L, w, h, self.crop_size, self.crop_size)
                    image_H = TF.crop(image_H, w, h, self.crop_size, self.crop_size)
    ...
    def randomRotation(self, image, image2, label, mode=Image.BICUBIC):
        """
        ...
        return image.rotate(random_angle*90, mode), image2.rotate(random_angle*90, mode), label.rotate(random_angle*90, Image.NEAREST)
...
class FundusSeg_Loader_Test(Dataset):
    ...
    def __getitem__(self, index):
        ...
        ## image_L = image_L.resize((384, 384), Image.BILINEAR)
        ## image_H = image_H.resize((384, 384), Image.BILINEAR)
        ## label = label.resize((384, 384), Image.NEAREST)
